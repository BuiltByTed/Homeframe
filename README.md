...
| iOS leaves a blurry/translucent strip above the app header. | One persistent shell owns and paints the top safe-area backing as part of the stable header, including iOS 26’s edge treatment. `AppShell` and `AppScrollView` stay above `RouterOutlet` so route changes cannot briefly expose a detached system blur layer. |
