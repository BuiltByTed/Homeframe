...
| A splash interrupts browser/desktop launches or its logo moves during iOS startup. | Branded splash defaults to installed mobile apps only. The native and HTML launch surfaces share artwork and physical-screen alignment, including the opaque iOS status-bar offset. Install icons default to `purpose: any`; platform masking is an explicit opt-in. |
...
| The startup logo begins centered, jumps toward a safe area, then moves back. | The generated Apple startup image and HTML splash center the logo on the same immutable full-screen canvas. Safe-area padding, measured shell height, and optional title layout cannot move the logo during handoff. |
