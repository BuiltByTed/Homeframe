...
| A service-worker update mixes old and new assets or strands users on a stale bundle. | Atomic build-ID caches, cache cleanup, offline navigation, and configurable prompt/automatic/manual activation keep a release internally consistent. Safe-point reload is configurable. |
...
The normative behavior and acceptance criteria are in [SPEC.md](./SPEC.md). Migration instructions are in [docs/adoption-runbook.md](./docs/adoption-runbook.md).
...
The repository also includes a native XCUITest harness in `tests/ios` for the installed-app viewport contract. Simulator coverage is a regression gate, not a substitute for the physical-device release matrix in the spec.
