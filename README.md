...
| Bottom navigation or a text composer teleports when the keyboard opens or closes, or page content shows behind the keyboard. | `ViewportDock keyboard="avoid"` animates with measured keyboard geometry, removes the Home-indicator inset while the keyboard owns that edge, and grows an opaque shell mask over the keyboard-owned rectangle. |
