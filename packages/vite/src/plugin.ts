...
function criticalCss(config: HomeframeConfig): string {
  ...
  return `:root{--hf-app-background:${background};--hf-color-scheme:${cssScheme};background:${background};color-scheme:var(--hf-color-scheme)}html,body,#homeframe-root{width:100%;height:100%;margin:0;overflow:hidden;background:var(--hf-app-background)}html,body{overscroll-behavior:none}#homeframe-boot-splash{position:fixed;z-index:2147483647;inset:0;min-height:var(--hf-shell-height,100%);display:grid;place-content:center;place-items:center;gap:16px;padding:env(safe-area-inset-top) env(safe-area-inset-right) env(safe-area-inset-bottom) env(safe-area-inset-left);color:CanvasText;background:var(--hf-app-background);font:600 17px/1.2 system-ui,-apple-system,sans-serif;transition:opacity 160ms ease}#homeframe-boot-splash img{width:96px;height:96px;object-fit:contain}:root[data-hf-ready=true]:not([data-hf-splash-visible]) #homeframe-boot-splash{visibility:hidden;opacity:0;pointer-events:none}:root[data-hf-splash-visible] #homeframe-boot-splash{visibility:visible;opacity:1}${adaptiveBackground}@media(prefers-reduced-motion:reduce){#homeframe-boot-splash{transition:none}}`;
}
