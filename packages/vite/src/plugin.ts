...
function bootScript(info: {
  ...
  return `(function(){var d=document.documentElement,v=window.visualViewport,ms=window.matchMedia,r=document.getElementById('homeframe-runtime-vars'),s,ios=navigator.standalone===true;try{s=r.sheet.cssRules[0].style}catch(e){s=d.style}window.__HOMEFRAME_BUILD__=${JSON.stringify(buildInfo)};d.dataset.hfReady='false';d.dataset.hfKeyboard='closed';d.dataset.hfDisplayMode=ms&&ms('(display-mode: fullscreen)').matches?'fullscreen':ms&&ms('(display-mode: standalone)').matches?'standalone':ms&&ms('(display-mode: minimal-ui)').matches?'minimal-ui':ios?'standalone':'browser';d.dataset.hfIosStandalone=ios?'true':'false';function m(){if(d.dataset.hfReady==='true')return;var w=v?v.width:window.innerWidth,h=v?v.height:window.innerHeight,x=v?v.offsetLeft:0,y=v?v.offsetTop:0,p=ios&&v&&isFinite(v.pageTop)&&v.pageTop>0?v.pageTop:0,z=v?v.scale:1,sh=window.innerHeight;if(ios){var ph=window.screen&&isFinite(window.screen.height)&&window.screen.height>0?window.screen.height:window.outerHeight;sh=Math.max(sh,ph)}s.setProperty('--hf-viewport-width',w+'px');s.setProperty('--hf-viewport-height',h+'px');s.setProperty('--hf-viewport-x',x+'px');s.setProperty('--hf-viewport-y',y+'px');s.setProperty('--hf-layout-viewport-top',p+'px');s.setProperty('--hf-shell-width',(d.dataset.hfDisplayMode==='browser'?x+w:Math.max(window.innerWidth,x+w))+'px');s.setProperty('--hf-shell-height',(d.dataset.hfDisplayMode==='browser'?y+h:Math.max(sh,y+h))+'px');s.setProperty('--hf-stable-width',window.innerWidth+'px');s.setProperty('--hf-stable-height',sh+'px');s.setProperty('--hf-input-min-font-size',(16/Math.min(Math.max(z,.1),1))+'px')}m();v&&v.addEventListener('resize',m,{passive:true});v&&v.addEventListener('scroll',m,{passive:true});window.addEventListener('resize',m,{passive:true})})()`;
}
