...
export function criticalCss(config: HomeframeConfig): string {
  ...
  const adaptiveBackground = configuredScheme === 'system'
    ...
    : '';
  const browserSplashPolicy = config.splash?.showInBrowserTabs === false
    ? `:root[data-hf-display-mode=browser] #homeframe-boot-splash{visibility:hidden;opacity:0;pointer-events:none}`
    : '';
